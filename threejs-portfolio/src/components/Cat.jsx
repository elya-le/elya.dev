...
const Cat = ({ animationName = 'Slow', origin = [0, 0, 0], ...props }) => {  // <------ Added `origin` prop
  const group = useRef();
  // const { nodes, materials, animations } = useGLTF('/public/assets/Cat_01_Idle_Faster.glb');
  const { nodes, materials, animations } = useGLTF('/public/assets/cat_09_modifiers.glb');
      ...
      <group position={origin}>  {/* <------ Updated to allow positioning based on `origin` prop */}
        <group ref={group}>
          ...
          <mesh
            ...
            />
            {/* Additional objects from new GLTF */}
            <mesh
              name="Chair"
              castShadow
              receiveShadow
...
// preload model
useGLTF.preload('/public/assets/cat_09_modifiers.glb');
